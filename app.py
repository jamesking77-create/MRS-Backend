...
def get_movies():
    try:
        num_movies = int(request.args.get('num_movies', 3))
        selected_movies = df.head(num_movies).to_dict(orient='records')
...
def recommend_movies():
    try:
        ...
        recommended_movies = df.iloc[similar_movie_indices][['id', 'title', 'overview']].to_dict(orient='records')
